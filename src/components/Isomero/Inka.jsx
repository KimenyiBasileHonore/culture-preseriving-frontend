...
export default function Inka() {
  ...
  const [boxesInka, setBoxesInka] = useState([]);

  useEffect(() => {
    // Fetch the data from the backend API and set it in the state
  ...
  const fetchDataFromBackend = async () => {
    try {
      ...
      const response = await fetch('/api/inka'); // Replace '/api/boxes' with the actual API endpoint URL
