...
function Ikenshkumwami() {
  ...
  const fetchUmwamiList = async () => {
    try {
      const response = await axios.get("http://localhost:4050/api/umwami/umwami");
  ...
  const createUmwami = async () => {
    try {
      await axios.post("http://localhost:4050/api/umwami/umwami", {
        title: newTitle,
        summary: newSummary,
        details: newDetails,
      });
  ...
  const updateUmwami = async (id, updatedData) => {
    try {
      await axios.put(`http://localhost:4050/api/umwami/umwami/${id}`, updatedData);
  ...
  const deleteUmwami = (id) => {
    ...
    axios.delete(`http://localhost:4050/api/umwami/delumwami/${id}`)
      .then(() => {
