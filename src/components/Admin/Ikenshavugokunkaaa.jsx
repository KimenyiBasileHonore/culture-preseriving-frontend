...
function Ikenshkungoma() {
  ...
  const fetchKungomaList = async () => {
    try {
      const response = await axios.get("http://localhost:4050/api/kunka/kunka");
  ...
  const createKungoma = async () => {
    ...
    try {
      await axios.post("http://localhost:4050/api/kunka/kunka", {
        title: newTitle,
        summary: newSummary,
        details: newDetails,
      });
  ...
  const updateKungoma = async (id, updatedData) => {
    try {
      await axios.put(`http://localhost:4050/api/kunka/kunka/${id}`, updatedData);
  ...
  const deleteKungoma = (id) => {
    ...
    axios
      .delete(`http://localhost:4050/api/kunka/kunka/${id}`)
      .then(() => {
      ...
      <div className={`collapsible ${isOpen ? 'open' : ''}`}>
        <div className="collapsible-header" onClick={toggleCollapsible}>
          <div className="header-content">
            <h1 className="title-font sm:text-2xl text-xl font-medium text-gray-900 mb-3">Ku bijyanye n’inka </h1>
